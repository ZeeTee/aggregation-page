...
npm test               # Vitest(131 个用例 / 10 个文件)
npm run smoke          # 冒烟:16 项状态码断言(--port 8080 打本机,--public 打线上)
```

实测:构建 **约 0.19 秒**(前端 0.16s + 服务端 0.03s),131 个测试约 **13 秒**,冒烟 23 项约 0.3 秒。
...
| GET | `/api/dsh/update` | 🔑 **需要 `X-Api-Key`**:DSH 版本检测与更新任务进度 |
| POST | `/api/dsh/update` | 🔑⚠️ **需要 `X-Api-Key`**:更新/回滚 DSH 并重启(**后台任务**),带 60 秒冷却 |
...
AUTH_RATE_LIMIT=30       # 这类接口独立限流(普通接口 60/分钟)
                        # ⚠️ 别调太低:「重启 DSH」靠轮询 login-url 等新令牌,
                        #    dsh 启动到打印 token 实测约 8.6 秒;5/分钟时轮询会在 token
                        #    出现前就被 429 挡住(见过这个故障)。爆破由失败预算兜底。
...
**有副作用的接口**(`POST /api/dsh/restart`、`POST /api/news/manual`、`POST /api/dsh/update`)额外要求:
...
| `tools/dsh-update/README.md` | 🤖 DSH 更新工具:为什么回滚放在工具站、版本通道的坑、安全约束 |
